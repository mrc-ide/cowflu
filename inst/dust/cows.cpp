...
public:
  ...
  static void initial(real_type time,
    ...
    const auto len_state = 5 * (shared.n_herds + shared.n_regions) + 3 * shared.n_regions;
    ...
    auto *cumul_true_outbreak_region = state_next + 5 * n + 2*shared.n_regions;
    ...
    std::transform(I_region, I_region + shared.n_regions, cumul_true_outbreak_region,
              [](int x) { return (x > 0) ? 1 : 0; });
  ...
  static void update(real_type time,
    ...
    const real_type* cumulative_true_outbreak_region = probability_test_pass_region + shared.n_regions;
    ...
    real_type* cumulative_true_outbreak_region_next = probability_test_pass_region_next + shared.n_regions;
    ...
    //Tally the cumulative number of herds in each region that have ever had an infected cow in
    for (size_t i = 0; i < shared.n_regions; ++i) {
      const size_t i_start = shared.region_start[i];
      const size_t i_end = shared.region_start[i + 1];
      size_t infected_herds_tally = 0;
      ...
      cumulative_true_outbreak_region_next[i] = cumulative_true_outbreak_region[i] + infected_herds_tally;
    }
